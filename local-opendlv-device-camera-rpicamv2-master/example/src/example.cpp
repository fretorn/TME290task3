...
int32_t main(int32_t argc, char **argv) {
  ...
  if ((0 == commandlineArguments.count("name")) || (0 == commandlineArguments.count("cid")) || (0 == commandlineArguments.count("traincnn"))) {
    ...
    while (od4.isRunning()) {
      ...
      cv::inRange(hsvImg, cv::Scalar(lowH, lowS, lowV, 0), cv::Scalar(highH, highS, highV, 0), threshImg);
      ...
      if (saveImage == 6) { //hsvImg
          std::cerr << argv[0] << "Saving hsvImg directly after line" << std::endl;
      ...
      if (saveImage == 11) {
        cv::inRange(scaledImage, cv::Scalar(lowH, lowS, lowV), cv::Scalar(highH, highS, highV), threshImg);
        std::cerr << argv[0] << "Saving hsvImg directly after line unscaled" << std::endl;
        std::string const FILENAME = "Unscaled" + std::to_string(i) + ".jpg";
        cv::imwrite(FILENAME, hsvImg);
        i++;
      }

      if (saveImage == 7) { //threshImg
          std::cerr << argv[0] << "Saving threshImg directly after line" << std::endl;
          std::string const FILENAME = std::to_string(i) + ".jpg";
          cv::imwrite(FILENAME, threshImg);
          i++;
      }

      if (saveImage == 10) { //threshImg
          std::cerr << argv[0] << "Saving threshImg directly after line" << std::endl;
      ...
      if (saveImage == 10) { //threshImg
          std::cerr << argv[0] << "Saving threshImg after Blue,dilate and erode directly after line" << std::endl;
          std::string const FILENAME = "threshImgAfterEffects" + std::to_string(i) + ".jpg";
          cv::imwrite(FILENAME, threshImg);
          i++;
      }
      ...
      if (saveImage == 9) { //print size of v3fCircles
          std::cerr << argv[0] << "Size of v3fCircles " << v3fCircles.size() << std::endl;
      }
      

      for (unsigned int j = 0; j < v3fCircles.size(); j++) {						// for each circle
        if (saveImage > 4) {
          std::cerr << argv[0] << "Inside for loop " << std::endl;
        }
