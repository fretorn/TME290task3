...
int32_t main(int32_t argc, char **argv) {
  ...
  if (0 == commandlineArguments.count("cid") || 0 == commandlineArguments.count("freq")) {
    ...
    //From camera
    auto onCameraEstimation{[&behavior](cluon::data::Envelope &&envelope)
      {
        auto cameraEstimation = cluon::extractMessage<opendlv::logic::sensation::Point>(std::move(envelope));
        // cameraDetectionAngle = cameraEstimation.azimuthAngle();
        // cameraDistance = cameraEstimation.distance();
        behavior.setAzimuthAngle(cameraEstimation.azimuthAngle());
        behavior.setDistance(cameraEstimation.distance());
    ...
    &minDistanceToCar, &maxDistanceToCar]() -> bool
      {
        ...
        if (VERBOSE) {
          std::cout << "Steer " << std::setw(6) << groundSteeringAngleRequest.groundSteering()
            ...
            // << " cameraDetectionAngle " << std::setw(6) << cameraDetectionAngle
            // << " cameraDistance " << std::setw(6) << cameraDistance
            // << " Right " << rightDistance
            << std::endl;
        }
